...
class repair_droid:
    ...
    def get_shortest_path_to_oxygen(self):
        neighbours = {k: [self._get_new_position(k, m + 1) for m in range(4) if v[m] != 0] for k, v in self._neighbours.items()}
        return self._find_shortest_path((0, 0), self._oxygen_position, neighbours)

    @staticmethod
    def _find_shortest_path(start, end, neighbours):
        import sys
...
if __name__ == '__main__':
    ...
    print(droid.get_shortest_path_to_oxygen())
